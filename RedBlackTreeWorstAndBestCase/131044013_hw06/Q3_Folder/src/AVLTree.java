...
public class AVLTree < E
    ...
    /**
     * Bir binarySearch tree alır ve balance olup olmadığını kontrol eder.
     * @param binarySearchTree
     */
    public AVLTree(BinarySearchTree<E> binarySearchTree){
        if (isBalanced(binarySearchTree.root)){
            System.out.println("Binary Search Tree is AVLTree");
        }
        else {
            System.out.println("Binary Search Tree is not AVLTree");
        }
    }

    /**
     * Node'dan başlayarak treenin balance olup olamdığını hesaplar.
     * @param local
     * @return boolean
     */
    private boolean isBalanced(Node<E> local)
    {
        ...
        lh = height(local.left);
        rh = height(local.right);

        if (Math.abs(lh - rh) <= 1
                && isBalanced(local.left)
                && isBalanced(local.right))
            return true;
    ...
    /**
     * local tree nin yüksekliğini hesaplar.
     * @param local
     * @return int
     */
    private int height(Node<E> local)
    {
        ...
        return 1 + Math.max(height(local.left), height(local.right));
    }
