...
public class BTree<E extends Comparable<E>>  implements SearchTree<E> {
    ...
    /**
     * İtemin hangi index e ekleneceğini bulur ve index return eder.
     * @param item
     * @param data
     * @param zero
     * @param rootsize
     * @return int
     */
    private int binarySearch (E item, E[] data, int zero, int rootsize ) {
        // Return the index of the leftmost element in this node that is not less
        // than target.
        int leftmost = zero, right = rootsize-1;
        while (leftmost <= right) {
            int index = (leftmost + right)/2;
            int comp = item.compareTo(data[index]);
            if (comp == 0)
                return index;
            else if (comp < 0)
                right = index - 1;
